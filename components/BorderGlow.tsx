...
function parseHSL(hslStr: string): { h: number; s: number; l: number } {
  ...
  if (!h || !s || !l) return { h: 40, s: 80, l: 80 };
...
function buildMeshGradients(colors: string[]): string[] {
  ...
  for (let i = 0; i < 7; i++) {
    const c = colors[Math.min(COLOR_MAP[i] ?? 0, colors.length - 1)] ?? colors[0] ?? '#000';
    gradients.push(`radial-gradient(at ${GRADIENT_POSITIONS[i]}, ${c} 0px, transparent 50%)`);
  }
  gradients.push(`linear-gradient(${colors[0] ?? '#000'} 0 100%)`);
  return gradients;
}
...
const BorderGlow: React.FC<BorderGlowProps> = ({
  ...
  useEffect(() => {
    ...
    let frame: number;
    const rotate = () => {
      setCursorAngle((prev) => (prev + 0.4) % 360);
  ...
  const colorSensitivity = edgeSensitivity + 10;
        ...
        style={
          {
            ...
            maskImage: [
              `conic-gradient(from ${angleDeg} at center, transparent 0%, black 15%, black 85%, transparent 100%)`,
